...
import org.peek.utils.email.MyAuthenticator;
...
public class MailSenderUtils  {      
     ...
     * @return
     */
    public static WebResult<Boolean> sendHtmlEmail(String toAddress, String ccAddress, String subject, String content) {
		MailSenderInfo mailInfo = new MailSenderInfo();
		setMailServerInfo(mailInfo);
		// 判断是否需要身份认证
		MyAuthenticator authenticator = null;
		Properties pro = mailInfo.getProperties();
		// 如果需要身份认证，则创建一个密码验证器
		...
		try {
			...
			// 设置邮件消息的主题
			if(!StringUtils.isEmpty(subject)){ // 如果标题为空，设置为标准内容
				mailMessage.setSubject(subject);
			}else{
				mailMessage.setSubject(EmailConstant.MAIL_TITLE);
			}
			// 设置邮件消息发送的时间
			mailMessage.setSentDate(new Date());
			// MiniMultipart类是一个容器类，包含MimeBodyPart类型的对象
			...
			// 发送邮件
			Transport.send(mailMessage);
			log.info("email send success,reciver:{},sendEmail:{}",toAddress,mailInfo.getFromAddress());
			return WebResult.ok(Boolean.TRUE);
		} catch (MessagingException ex) {
			log.error(ex.getMessage(),ex);
			return WebResult.fail("send email fail,error:"+ex.getMessage());
		}
	 ...
	 * @param mailInfo
	 */
	private static void setMailServerInfo(MailSenderInfo mailInfo) {
		...
		// mailInfo.setSubject(EmailConstant.MAIL_TITLE);
